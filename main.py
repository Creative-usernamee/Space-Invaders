...
class Laser(pygame.sprite.Sprite):
    ...
    # Стирает лазеры, выходящие за экран
...
class Game:
    def __init__(self):
        global SCORE
        # Максимальное кол-во очков за уровень
    ...
    # Кнопка: экран выбора уровней - главное меню
    ...
    # Загрузка случайного уровня (пришлось вынести в отдельную функцию)
    ...
    # Проверка на столкновение врагов с краем экрана
    ...
    # Спуск вниз при касании края экрана
    def move_down(self):
        if self.aliens:
            for alien in self.aliens.sprites():
                alien.rect.y += 2

    # Выстрел случайного врага
    def shoot(self):
    ...
    # Отображение жизней и очков в уровне
    ...
    # Функция закрытия экрана победы, открывает следующий уровень
    def next_level(self):
        global SCORE
        screen.fill((35, 35, 35))
        self.current_level += 1
...
if __name__ == '__main__':
    ...
    # Выстрел врага
    alien_laser_ready = 0
    # Зацикливаем выстрел, каждые 500 мс создается ивент alien_laser_ready
    pygame.time.set_timer(alien_laser_ready, 500)
    time_start = 0
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            # При ивенте alien_laser_ready враг стреляет
